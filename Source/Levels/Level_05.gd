...
func _process(delta):
	...
	if get_node("Balances/Balance_If_Else_Diamond_If").next_animation and get_node("Balances/Balance_If_Else_Diamond_Else").next_animation:
		get_node("Door").level_clear = true
	
	if get_node("Door").door_open:
		SceneChanger.change_scene("res://Source/Cutscenes/Chapter09/HiddenCave04.tscn")
	pass
